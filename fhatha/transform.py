...
class FastAccurateHankel():
    def __init__(self, n_samples, r_max=1):
        ...
        self.kr = grid * 2 * np.pi / (self.x0 * self.r_max) 
    ...
    def generate_grid(self) -> tuple:
        ...
        x0 = np.exp(-alpha*(self.n_samples))/2 * (1+np.exp(alpha)) # FIXME divide by zero warning
    ...
    def prepare_js(self):
        x_padded = self.x0 / self.r_max * \
        np.exp((np.arange((2 * self.n_samples)) - self.n_samples + 1) * self.alpha)
        return jv(1, 2 * np.pi * x_padded / self.x0)
    ...
    def fht(self, fs):
        ...
        phis[0] *= self.k0 
        return self.correlate(phis, self.js)[:self.n_samples] * self.x0 / self.grid / self.r_max**3
        
    def ifht(self, fs):
        return self.fht(fs) / ((self.x0/self.r_max**3)**2)