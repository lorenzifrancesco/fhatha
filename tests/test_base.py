...
def test_compatibility_with_magni(fresnel = 10, clip_reference = 1, func = magni):
    ...
    fh = md.FastAccurateHankel(n_samples, r_max=1/r_max)
    ...
    exactc = 'red'
    color1b = 'black'
    ax1.plot(log_k, np.real(exact)[:n_samples], lw=1.3, ls="--", color=exactc, label="EXACT")
...
if __name__ == "__main__":
    test_compatibility_with_magni(clip_reference=2, func=gaussian)
